// javascript
// File: `test/ha-atrea-recuperation-card.test.js`
...
describe('ha-atrea-recuperation-card', () => {
  it('is defined', async () => {
    const el = await fixture(html`<ha-atrea-recuperation-card></ha-atrea-recuperation-card>`);
    expect(el).to.exist;
  });

  it('renders provided title', async () => {
    const el = await fixture(html`<ha-atrea-recuperation-card></ha-atrea-recuperation-card>`);
    el.setConfig({ title: 'Test Title', entity_climate: 'climate.test' });
  ...
  it('shows default target slider with configured min/max/step', async () => {
    const el = await fixture(html`<ha-atrea-recuperation-card></ha-atrea-recuperation-card>`);
    el.setConfig({ title: 'Test', entity_climate: 'climate.test', min_temp: 10, max_temp: 30, temp_step: 1 });
    await el.updateComplete;
